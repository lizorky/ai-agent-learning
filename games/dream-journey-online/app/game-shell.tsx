...
import { SKILL_PROFILES, STRIKE_BAND } from '@/game/combat-shapes.mjs';

type HeroId = 'vanguard' | 'sage' | 'guardian' | 'ranger';
type SkillId = 'flurry-fist' | 'rising-punch' | 'blazing-step';
...
const skills: Array<{ id: SkillId; key: string; name: string; cost: number; detail: string }> = [
  { id: 'flurry-fist', key: 'U', name: '疾风连拳', cost: 12, detail: '连续拳风击退近敌' },
  { id: 'rising-punch', key: 'I', name: '升龙拳', cost: 20, detail: '重拳挑飞前方敌人' },
  { id: 'blazing-step', key: 'O', name: '烈焰冲拳', cost: 28, detail: '踏火突进并轰出重拳' },
];
...
export default function GameShell() {
  ...
  useEffect(() => {
    ...
    function handleKey(event: KeyboardEvent, pressed: boolean) {
      ...
      const skillIndex = ['KeyU', 'KeyI', 'KeyO'].indexOf(event.code);
      ...
      {roomCode && roomState ? (
        <section className="game-layout">
          ...
          <div className="arena-wrap">
            <div className="arena-heading">
              ...
              <div className="arena-actions"><span>{players.length} / 4 名玩家</span><button className={`pause-button${localPauseOwner ? ' active' : ''}`} disabled={roomState.paused && !localPauseOwner} onClick={() => { setInventoryOpen(false); setPauseMenuOpen((open) => !open); }}>{localPauseOwner ? '继续' : roomState.paused ? '等待' : '暂停'} <kbd>Esc</kbd></button><button className="inventory-button" onClick={() => { setPauseMenuOpen(false); setInventoryOpen(true); }}>行囊 <kbd>Tab</kbd></button></div>
            </div>
            ...
            <div className="canvas-stage">
              ...
              {roomState.stage.state === 'victory' && (
                <div className="victory-overlay">
                  <p>CHAPTER CLEAR</p>
                  <h3>云栈古道 · 平定</h3>
                  <span>山行布衣、养元露与山妖残晶已放入全队行囊</span>
            ...
            <div className="game-controls desktop-controls"><kbd>A</kbd><kbd>D</kbd> 移动 <kbd>K</kbd> 跳跃 <kbd>J</kbd> 三段拳击 <kbd>U</kbd><kbd>I</kbd><kbd>O</kbd> 拳法 <kbd>Tab</kbd> 行囊 <kbd>Esc</kbd> 暂停</div>
        ...
        <section className="hero-grid">
          ...
          <div className="stage-panel" aria-label="云栈古道关卡预览">
            ...
            <div className="player-preview" style={{ background: currentHero.color }} aria-hidden="true"><HeroPortrait hero={currentHero.id} className="preview-face" /></div>
...
function GameCanvas({ state, playerId }: { state: RoomState; playerId: string }) {
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      const now = stateRef.current.animationTime || 0;
      const shaking = !stateRef.current.paused && (stateRef.current.combatEffects || []).some((effect) => effect.type === 'impact' && effect.life > 0.88);
      ...
      for (const drop of stateRef.current.drops) drawGroundDrop(context, drop, now);
      for (const enemy of stateRef.current.enemies) drawEnemy(context, enemy, now);
      for (const projectile of stateRef.current.projectiles || []) drawProjectile(context, projectile);
      for (const player of stateRef.current.players) drawPlayer(context, player, player.id === playerId, now);
...
function drawPlayer(context: CanvasRenderingContext2D, player: PlayerState, isCurrent: boolean, now: number) {
  const vanguardAtlas = player.hero === 'vanguard' ? getGameImage('/assets/characters/wukong-actions.png') : null;
  const vanguardBody = player.hero === 'vanguard' ? getGameImage('/assets/characters/wukong-body.png') : null;
  context.save();
  ...
  if (vanguardAtlas || vanguardBody) {
    if (player.invulnerable > 0 && Math.floor(now / 70) % 2 === 0) context.globalAlpha = 0.45;
    if (vanguardAtlas) {
      const frame = wukongAnimationFrame(player, now);
      drawWukongFrame(context, vanguardAtlas, frame);
      if ((player.attacking || player.skill) && isImpactPhase(player)) drawVisibleFist(context, player);
    } else if (vanguardBody) {
      const strideBob = player.onGround && Math.abs(player.vx) > 28 ? Math.sin(now / 85) * 4 : 0;
      context.save();
      context.translate(0, strideBob);
      context.drawImage(vanguardBody, -105, -218, 210, 220);
      context.restore();
    }
  } else {
  ...
  const labelY = player.hero === 'vanguard' ? player.y - 214 : player.y - 112;
...
function drawPlayerAttackEffect(context: CanvasRenderingContext2D, player: PlayerState) {
  ...
  context.save();
  const fistX = Math.min(reach, 82);
  context.strokeStyle = `rgba(255,196,62,${0.72 - progress * 0.28})`;
  context.shadowColor = '#ff9d26';
  context.shadowBlur = 12;
  context.lineWidth = player.comboStep === 3 ? 9 : 6;
  context.beginPath();
  context.moveTo(32, STRIKE_BAND.center);
  context.lineTo(fistX, STRIKE_BAND.center);
  context.stroke();
  context.beginPath();
  context.arc(fistX, STRIKE_BAND.center, player.comboStep === 3 ? 21 : 15, 0, Math.PI * 2);
  context.stroke();
  context.restore();
}
...
function drawPlayerSkillEffect(context: CanvasRenderingContext2D, player: PlayerState) {
  ...
  const profile = player.skill ? SKILL_PROFILES[player.skill] : null;
  context.save();
  if (player.skill === 'blazing-step') {
    ...
    context.strokeStyle = player.skill === 'rising-punch' ? 'rgba(255,229,113,.78)' : 'rgba(255,143,45,.72)';
    context.lineWidth = player.skill === 'rising-punch' ? 12 : 8;
    context.beginPath();
    context.arc(20, STRIKE_BAND.center, Math.min(profile?.range || 105, 42 + progress * 62), -1.25, 0.8);
    context.stroke();
  }
  context.restore();
}

function wukongAnimationFrame(player: PlayerState, now: number) {
  if (player.respawnTimer > 0) return 9;
  if (player.hitStun > 0) return 7;
  if (player.skill) return skillAnimationFrame(player);
  if (player.attacking) return actionAnimationFrame(player.attackTimer, player.attackDuration, player.attackImpactAt, player.comboStep);
  if (!player.onGround) return 3;
  if (Math.abs(player.vx) > 28) return Math.floor(now / 105) % 2 ? 1 : 2;
  return Math.floor(now / 900) % 2 ? 0 : 10;
}

function actionAnimationFrame(timer: number, duration: number, impactAt: number, comboStep: number) {
  const frames = comboStep === 2 ? [0, 6, 8] : comboStep === 3 ? [4, 5, 6] : [4, 5, 0];
  if (duration <= 0 || timer > impactAt + 0.045) return frames[0];
  if (timer > Math.max(0, impactAt - 0.1)) return frames[1];
  return frames[2];
}

function skillAnimationFrame(player: PlayerState) {
  if (!player.skill) return 0;
  const profile = SKILL_PROFILES[player.skill];
  if (player.skill === 'flurry-fist' && player.skillTimer <= profile.impactAt + 0.12) {
    return Math.floor(player.skillTimer / 0.055) % 2 ? 5 : 6;
  }
  const combo = player.skill === 'rising-punch' ? 2 : 3;
  return actionAnimationFrame(player.skillTimer, player.skillDuration, profile.impactAt, combo);
}

function isImpactPhase(player: PlayerState) {
  const impactAt = player.skill ? SKILL_PROFILES[player.skill].impactAt : player.attackImpactAt;
  const timer = player.skill ? player.skillTimer : player.attackTimer;
  return timer <= impactAt + 0.1 && timer >= Math.max(0, impactAt - 0.11);
}

function drawVisibleFist(context: CanvasRenderingContext2D, player: PlayerState) {
  const combo = player.skill === 'rising-punch' ? 2 : player.skill === 'blazing-step' ? 3 : player.comboStep || 1;
  const fist = combo === 2 ? { x: 47, y: -118, angle: -0.45 } : combo === 3 ? { x: 73, y: -72, angle: 0.08 } : { x: 68, y: -82, angle: 0 };
  context.save();
  context.translate(fist.x, fist.y);
  context.rotate(fist.angle);
  context.fillStyle = '#17171b';
  context.strokeStyle = '#2d211d';
  context.lineWidth = 3;
  context.beginPath();
  context.roundRect(-18, -10, 18, 20, 6);
  context.fill();
  context.stroke();
  context.fillStyle = '#efc29d';
  context.beginPath();
  context.roundRect(-2, -12, 22, 24, 8);
  context.fill();
  context.stroke();
  context.restore();
}

function drawWukongFrame(context: CanvasRenderingContext2D, atlas: HTMLImageElement, frame: number) {
  ...
  context.drawImage(atlas, sourceX, sourceY, cellWidth, cellHeight, -110, -220, 220, 220);
}

function drawGroundDrop(context: CanvasRenderingContext2D, drop: RoomState['drops'][number], now: number) {
  const item = getItemDefinition(drop.itemId);
  if (!item) return;
  const color = rarityMeta[item.rarity].color;
  const bob = Math.sin(now / 260 + drop.x) * 5;
  context.save();
...
function drawEnemy(context: CanvasRenderingContext2D, enemy: EnemyState, now: number) {
  ...
  if (enemySheet) {
    ...
    const frame = enemyAnimationFrame(enemy, now);
...
function enemyAnimationFrame(enemy: EnemyState, now: number) {
  if (enemy.dead) return 7;
  if (enemy.hitStun > 0) return 6;
  if (enemy.attacking) return enemy.attackTimer > 0.24 ? 4 : 5;
  if (Math.abs(enemy.vx) > 18) return Math.floor(now / 150) % 2 ? 1 : 2;
  return 0;
}
