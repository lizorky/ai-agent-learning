...
const inRange = await waitFor(socket, (message) => {
  ...
  return player && enemy && Math.abs(enemy.x - player.x) < 82;
});
...
socket.send(JSON.stringify({ type: 'cast_skill', skillId: 'flurry-fist' }));

const skillState = await waitFor(socket, (message) => {
  if (message.type !== 'state') return false;
  const player = message.state.players.find((entry) => entry.id === playerId);
  const enemy = message.state.enemies[0];
  return player?.skillCooldowns?.['flurry-fist'] > 0 && enemy?.hp < beforeEnemy.hp;
});
...
assert.ok(afterPlayer.skillCooldowns['flurry-fist'] > 2, 'skill should start a cooldown');
...
const basicReady = await waitFor(socket, (message) => {
  ...
  return forwardDistance > 38 && forwardDistance < 68;
});
...
assert.ok(impactPlayer.attackTimer <= impactPlayer.attackImpactAt + 0.02, 'basic damage must land on the fist impact frame');
assert.ok(impactPlayer.attackStart >= 20, 'basic attack box must start in front of the player hurt box');

socket.close();
console.log('Authoritative fist-skill combat and forward impact hitbox smoke test passed.');
