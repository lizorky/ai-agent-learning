...
import { SKILL_PROFILES, fistHitsEnemy } from '../game/combat-shapes.mjs';
...
const SKILLS = SKILL_PROFILES;
const BASIC_ATTACKS = {
  vanguard: [
    { duration: 0.3, impactAt: 0.13, start: 20, reach: 72 },
    { duration: 0.34, impactAt: 0.15, start: 22, reach: 82 },
    { duration: 0.42, impactAt: 0.19, start: 24, reach: 96 },
  ],
  default: [
...
function simulateRoom(room, delta) {
  ...
  room.animationTime += delta * 1000;
  const groundY = 440;
  const now = room.animationTime;
  ...
  for (const player of room.players.values()) {
    ...
    if (movement !== 0 && player.attacking <= 0) player.facing = Math.sign(movement);
    ...
    if ((player.input.attack || player.attackBuffer > 0) && player.attacking <= 0 && player.attackCooldown <= 0 && player.hitStun <= 0 && !player.skill) {
    ...
    if (player.skill) {
      ...
      if (player.skill === 'blazing-step' && player.skillTimer > 0.18) {
      ...
      if (!player.skillHit && player.skillTimer <= skill.impactAt) {
        ...
        hitEnemies(room, player, damage, skill.range, skill.knockback, 0.32, skill.maxTargets, false, skill.start);
...
function hitEnemies(room, player, damage, range, knockback, hitStun, maxTargets = 1, critical = false, forwardStart = 24) {
  const candidates = room.enemies
    .filter((enemy) => fistHitsEnemy(player, enemy, forwardStart, range))
...
function addCombatEffect(room, type, x, y, value, color, critical = false) {
  room.combatEffects.push({ id: randomUUID(), type, x, y, value, color, critical, createdAt: room.animationTime });
}
...
function completeStage(room) {
  ...
  for (const player of room.players.values()) {
    ...
    if ((player.inventory.get('mountain-cloth-tunic') || 0) < 1) addInventoryItem(player, 'mountain-cloth-tunic', 1);
...
function serializeRoom(room) {
  return {
    ...
    animationTime: round(room.animationTime),
    sentAt: Date.now(),
    ...
    combatEffects: room.combatEffects.map((effect) => ({
      ...
      life: round(Math.max(0, 1.1 - (room.animationTime - effect.createdAt) / 1000)),
    })),
...
function restorePlayerProfile(player, profile) {
  ...
  // Chapter one is an unarmed tutorial. Weapons stay in the bag until weapon styles unlock later.
...
function spawnEnemyLoot(room, player, enemy) {
  ...
  for (let index = 0; index < dropCount; index += 1) {
    ...
    room.drops.push({
      ...
      y: 420,
      ownerId: player.id,
      createdAt: room.animationTime,
    });
  }
}

function rollLootItem() {
  const availableItems = ITEM_CATALOG.filter((item) => item.slot !== 'weapon');
  const totalWeight = availableItems.reduce((sum, item) => sum + Math.max(0, item.dropWeight), 0);
